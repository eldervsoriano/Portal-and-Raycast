...
public class PortalGun : MonoBehaviour
{
    ...
    public float maxRayDistance = 50f; // Maximum distance the ray can travel
    public float pickUpDistance = 4f; // Maximum distance to pick up objects
    public float dropDistance = 4f; // Maximum distance to drop objects
    ...
    void Update()
    {
        ...
        // Move BluePortal with left click if the target has the tag "Portable"
        if (Input.GetMouseButtonDown(0))
        {
            if (hit.collider != null && hit.collider.CompareTag("Portable"))
            {
        ...
        // Move OrangePortal with right click if the target has the tag "Portable"
        ...
        // Pick up and place objects with the F key
